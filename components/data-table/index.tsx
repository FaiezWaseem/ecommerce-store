...
export default function DataTable( { data , INITIAL_VISIBLE_COLUMNS , searchKey , renderCell , columns , onAddNewClick }  : PropsDataTable ) {
  ...
  const filteredItems = React.useMemo(() => {
    ...
    if (hasSearchFilter) {
      filtereddata = filtereddata.filter((dataItem : Object) =>
        dataItem[searchKey]?.toLowerCase().includes(filterValue.toLowerCase()),
      );
    }
    if (statusFilter !== "all" && Array.from(statusFilter).length !== statusOptions.length) {
      filtereddata = filtereddata.filter((dataItem) =>
        Array.from(statusFilter).includes(dataItem?.status),
      );
    }
      ...
      <div className="flex flex-col gap-4">
        <div className="flex justify-between gap-3 items-end flex-wrap">
          ...
          <div className="flex gap-3 flex-wrap">
            ...
            <Button color="primary" endContent={<PlusIcon />} onClick={onAddNewClick}>
              Add New
            </Button>
          </div>
        </div>
